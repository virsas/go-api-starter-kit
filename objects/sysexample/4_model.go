...
const exampleGetQuery string = "SELECT id FROM examples WHERE id=$1;"
const exampleListQuery string = "SELECT id, name, updated_at, created_at FROM examples ORDER BY id DESC;"
const exampleShowQuery string = "SELECT id, name, updated_at, created_at FROM examples WHERE id=$1;"
...
func (m *model) list() ([]Example, error) {
	var example Example
...
func (m *model) create(c context.Context, model ExampleInput) error {
	...
	_, err = exampleInsertQuery.ExecContext(c, model.Name, 1, 1, time.Now(), time.Now())
	if err != nil {
		tx.Rollback()
		m.log.Error(err.Error())
		return vars.StatusDBError(err)
...
func (m *model) show(id int64) (Example, error) {
	var example Example
...
func (m *model) update(c context.Context, id int64, model ExampleInput) error {
	tx, err := m.db.BeginTx(c, nil)
	if err != nil {
		m.log.Error(err.Error())
		return vars.StatusServerError(err)
